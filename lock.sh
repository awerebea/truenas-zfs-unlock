...
is_unlocked() {
  ...
  if [ "${locked}" = 'false' ]; then
    ...
    echo "${pool}/${dataset} is already locked"
    return 1
  else
...
lock (){
  ...
  http_code="$(
    ...
    "https://${TRUENAS_HOST}/api/v2.0/pool/dataset/lock" \
    -H 'accept: */*' \
    -H "Authorization: Bearer ${api_token}" \
  ...
  if [ "${http_code}" -ne 200 ]; then
    ...
    echo "Got a 200 code in response to locking request. Does not imply success unfortunately"
...
for zfs_var in ${zfs_env_vars}; do
  ...
  # Expecting 'ZFS__<pool name>__<dataset__name>=<optional>'
  # The 'ZFS__' environment variable value (<zfs passphrase>) is not required for the `unlock` command.
  passphrase=$(echo "${zfs_var}" | cut -d= -f2) # NOTE: optional
  ...
  printf "\nDiscovered %s/%s, attempting to lock now\n" "${pool}" "${dataset}"
  # Check if locked
